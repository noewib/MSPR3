...
class DataPipeline:
                ...
                if records:
                    ...
                    for r in records:
                        ...
                        data_list.append({
                            ...
                            'temperature': fields.get('temperature', self._generate_simulated_temp(pd.to_datetime(fields.get('date_heure'))))
                        })
                    df = pd.DataFrame(data_list)
                    df = df.dropna(subset=['consommation']).sort_values('datetime').reset_index(drop=True)
                    if not df.empty:
                        return df
        except Exception as e:
            print(f"Warning: ODRE API call failed ({e}). Falling back to simulated data.")
    ...
    def generate_historical_data(self, start_date=None, end_date=None, days=365) -> pd.DataFrame:
        """
        ...
        temp_noise = 2.0 * np.sin(np.arange(len(df)) / 100) + np.random.normal(0, 0.3, len(df))
        ...
        thermosensitivity = df['temperature'].apply(lambda t: max(0.0, thermo_threshold - t) * 1800.0)
        
        # Daily cycle (peak around 8h-13h and 18h-21h)
        hour = df['datetime'].dt.hour + df['datetime'].dt.minute / 60.0
        daily_factor = (
            5000.0 * np.sin(2 * np.pi * (hour - 6) / 24.0) +
            3000.0 * np.sin(4 * np.pi * (hour - 15) / 24.0)
        )
        ...
        df['consommation'] = base + thermosensitivity + daily_factor + weekly_factor + holiday_factor + noise
    ...
    def feature_engineering(self, df: pd.DataFrame, is_training: bool = True) -> pd.DataFrame:
        ...
        if HAS_HOLIDAYS:
            ...
            df['is_holiday'] = df['datetime'].dt.date.apply(lambda d: d in fr_holidays).astype(int)
        else:
            # Fallback simple holiday check (approximate major ones)
            # Jan 1, May 1, May 8, July 14, Aug 15, Nov 1, Nov 11, Dec 25
            major_holidays = [(1,1), (5,1), (5,8), (7,14), (8,15), (11,1), (11,11), (12,25)]
            df['is_holiday'] = df['datetime'].apply(
                lambda row: 1 if (row.month, row.day) in major_holidays else 0
            ).astype(int)
        ...
        df['temp_roll_mean_3h'] = df['temperature'].rolling(window=6, min_periods=1).mean()
        df['temp_roll_mean_6h'] = df['temperature'].rolling(window=12, min_periods=1).mean()
