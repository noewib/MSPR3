...
def main():
    ...
    processed_df = pipeline.feature_engineering(raw_df, is_training=True)
    ...
    models = {
        ...
        "RandomForest": RandomForestRegressor(n_estimators=30, max_depth=10, random_state=42, n_jobs=-1),
        "KNeighbors": KNeighborsRegressor(n_neighbors=5, weights='distance'),
        "RBFN": RadialBasisFunctionNetwork(n_centers=30, gamma='scale', alpha=0.1, random_state=42)
    }
    ...
    for name, model in models.items():
        ...
        print(f"[{name}] Done. R2: {r2:.4f} | MAPE: {mape*100:.2f}% | Accuracy+/-5%: {accuracy_5*100:.2f}% | Train Time: {train_time:.3f}s")
