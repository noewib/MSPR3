    ...
    def calculate_drift(
        ...
        if HAS_EVIDENTLY:
            try:
                ...
                print(
                    "Evidently AI HTML drift report saved to docs/monitoring/data_drift_report.html."
                )
            except Exception as e:
                print(
                    f"Error generating Evidently report: {e}. Falling back to statistical tests."
                )
...
def main():
    ...
    current_drifted_df["consommation"] = current_drifted_df["consommation"] - (
        8.0 * 1800.0
    )  # heating drops
    ...
    results_nominal = detector.calculate_drift(
        reference_df, current_nominal_df, features
    )
    ...
    results_drifted = detector.calculate_drift(
        reference_df, current_drifted_df, features
    )
