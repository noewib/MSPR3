...
def train_challenger(**kwargs):
    ...
    challenger_model = RandomForestRegressor(n_estimators=40, max_depth=12, random_state=42)
...
def evaluate_and_compare(**kwargs):
    ...
    if mape_chal < mape_champ and mape_chal <= 0.05:
        ...
        promote_challenger()
        return f"Promoted: Challenger ({mape_chal:.4f}) beat Champion ({mape_champ:.4f})."
    else:
...
if not isinstance(task_extract, MockPythonOperator):
    ...
    print("Mock DAG structure initialized: task_extract >> task_train_challenger >> task_evaluate_compare")
